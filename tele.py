...
'Sec-Fetch-Site': 'same-origin'}
        ...
        for i in reqemail:
            i=i['user']['username']
            print (i)
            with open('user.txt','a') as generall:
                generall.write(f"{i}\n")
...
def hash(message):
	if message.chat.id == id:
		bot.send_message(id,text="جار سحب يوزرات يرجوو الانتضار ")
		name=message.text.split(' ')[1]
		url ="https://i.instagram.com/api/v1/tags/web_info/?tag_name={name}"
...
def delete(message):
	if message.chat.id ==id:
		...
		import os 
		os.system("rm user.txt")
		bot.send_message(id,text="تم حذف الملف اليوزرات ")
